...
void Twi::step() {
  DBG_MD(APP, ("TWI Step %d: %d, stat: %d\n", RTC.CNT, state_, TWI0.MSTATUS));
  switch(state_) {
  case TWI_STATE_IDLE:
  ...
  case TWI_STATE_BYTES:
    if ((TWI0.MSTATUS & (TWI_BUSERR_bm | TWI_ARBLOST_bm)) != 0) {
      error_ = true;
    ...
    if (idx_ == len_ - 1) {
      ...
      DBG_MD(APP, ("TWI Bytes, stop MSTATUS: %d\n", TWI0.MSTATUS));
    }
    TWI0.MDATA = data_ptr_[++idx_];
    DBG_MD(APP, ("TWI Bytes: %d MSTATUS: %d\n", state_, TWI0.MSTATUS));
    break;

  case TWI_STATE_STOP:
    DBG_MD(APP, ("TWI Stop: entry: %d, MSTATUS: %d\n", state_, TWI0.MSTATUS));
    state_ = TWI_STATE_IDLE;
    TWI0.MCTRLB = TWI_MCMD_STOP_gc;
    DBG_MD(APP, ("TWI Stop: state: %d, MSTATUS: %d\n", state_, TWI0.MSTATUS));
    break;
  }
}
...
void Twi::WaitForIdle() {
  while(state_ != TWI_STATE_IDLE) {
    // DBG_MD(APP, ("Twi State: %d err: %d\n", state_, error_));
  }
}

void Twi::MasterSendBytes(u8_t addr, const u8_t* data, int len) {
  DBG_MD(APP, ("TWI Send: %d - CtrlA: %d\n", state_, TWI0.MCTRLA));
  WaitForIdle();
  DBG_MD(APP, ("TWI Send Idle: state: %d, MSTATUS: %d\n", state_, TWI0.MSTATUS));
  if ((TWI0.MSTATUS & TWI_BUSSTATE_gm) == 0) {
    TWI0.MSTATUS = TWI_BUSSTATE_IDLE_gc;
    DBG_MD(APP, ("TWI Send Force Idle MSTATUS: %d\n", state_, TWI0.MSTATUS));
  }
  error_ = false;
  data_ptr_ = data;
  len_ = len;
  ...
  DBG_MD(APP, ("TWI MADDR: %d\n", RTC.CNT));
  TWI0.MADDR = (addr & 0xFE); // clear low bit 0 = W, 1 = R
  DBG_MD(APP, ("TWI Send Return %d: state: %d, MSTATUS: %d\n",
               RTC.CNT, state_, TWI0.MSTATUS));
}
